...
def download_and_verify_zips(server):
    ...
    failure_file_name = 'failure_{:%Y_%b_%d_%H_%M_%S}'.format(datetime.datetime.now())
    ...
    for sample in sample_array:
        try:
            ...
            #escape special characters
                escaped_url= urllib.parse.quote(folder)
                time.sleep(2.0)
                call(["curl","-X","GET","-H", "'Cache-Control: no-cache'","-O", server + escaped_url])
                escaped_zip_name = urllib.parse.quote(zipfile)
