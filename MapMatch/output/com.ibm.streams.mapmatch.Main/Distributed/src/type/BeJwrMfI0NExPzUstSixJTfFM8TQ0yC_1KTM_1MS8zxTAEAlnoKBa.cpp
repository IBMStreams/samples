...
#include "BeJwrMfI0NExPzUstSixJTfFM8TQ0yC_1KTM_1MS8zxTAEAlnoKBa.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<int64 generatedId,int64 originalId>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("generatedId");
        TypeOffset t(MY_OFFSETOF(generatedId_, tm), 
    ...
    {
        std::string s("originalId");
        TypeOffset t(MY_OFFSETOF(originalId_, tm), 
                     Meta::Type::typeOf<SPL::int64 >(), 
                     &typeid(SPL::int64));
        addMapping(*tm, t, s, 1);
    }
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "generatedId") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, generatedId_);
   else
     istr >> generatedId_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "originalId") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, originalId_);
   else
     istr >> originalId_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, generatedId_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   if (c != ',') { istr.setstate(std::ios_base::failbit); return; }
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, originalId_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{'
         << "generatedId=" << get_generatedId()  << ","
         << "originalId=" << get_originalId()  
         << '}';
}
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "generatedId=", get_generatedId()) << ",";
    SPL::serializeWithPrecision(ostr << "originalId=", get_originalId()) ;
    ostr << '}';
}
