// t2I11generatedIdI10originalId


#ifndef BEJWRMFI0NEXPZUSTSIXJTFFM8TQ0YC_1KTM_1MS8ZXTAEALNOKBA_H_
#define BEJWRMFI0NEXPZUSTSIXJTFFM8TQ0YC_1KTM_1MS8ZXTAEALNOKBA_H_
...
#define SELF BeJwrMfI0NExPzUstSixJTfFM8TQ0yC_1KTM_1MS8zxTAEAlnoKBa
...
public:
    ...
    typedef SPL::int64 generatedId_type;
    typedef SPL::int64 originalId_type;

    enum { num_attributes = 2 } ;
    
    SELF() : Tuple(), generatedId_(), originalId_() {}
    SELF(const Self & ot) : Tuple(), generatedId_(ot.generatedId_), originalId_(ot.originalId_) 
      { constructPayload(ot); }
    SELF(const generatedId_type & _generatedId, const originalId_type & _originalId) : Tuple(), generatedId_(_generatedId), originalId_(_originalId) { }
    ...
    generatedId_type & get_generatedId() { return generatedId_; }
    const generatedId_type & get_generatedId() const { return generatedId_; }
    void set_generatedId(const generatedId_type & _generatedId) { generatedId_ = _generatedId; }
    
    originalId_type & get_originalId() { return originalId_; }
    const originalId_type & get_originalId() const { return originalId_; }
    void set_originalId(const originalId_type & _originalId) { originalId_ = _originalId; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<generatedId_type >()(generatedId_);
        s = 37 * s + std::tr1::hash<originalId_type >()(originalId_);
        return s;
    }
    ...
    { 
        generatedId_ = ot.generatedId_;
        originalId_ = ot.originalId_; 
        assignPayload(ot);
        return *this; 
    }
    ...
    void swap(SELF & ot) 
    { 
        std::swap(generatedId_, ot.generatedId_);
        std::swap(originalId_, ot.originalId_);
...
private:
    ...
    generatedId_type generatedId_;
    originalId_type originalId_;
...
#undef SELF
#endif // BEJWRMFI0NEXPZUSTSIXJTFFM8TQ0YC_1KTM_1MS8ZXTAEALNOKBA_H_ 
